...
class User extends Authenticatable
{
    ...
    */
    ...
    public function getAppData($city_id,$trending = false)
    {
        ...
        //Trae tanto los abiertos y los cerrados
        // $res = User::leftjoin('user_category', 'users.category_id', '=', 'user_category.id')
        //     ->where('users.status',0)->where('users.city_id',$city_id);

        /** Response para tiendas de un banner específico */
        if (isset($_GET['banner']))
        {
            $bannerId = $_GET['banner'];
            $res = User::leftjoin('banner_store', 'users.id', '=', 'banner_store.store_id')->leftjoin('user_category', 'users.category_id', '=', 'user_category.id')
            ->where('users.open', 0)->where('users.status',0)->where('banner_store.banner_id',$bannerId);
            ...
            {
                $res = User::where('trending', 1)->where('users.city_id',$city_id)->leftjoin('user_category', 'users.category_id', '=', 'user_category.id')
            ...
            if (isset($_GET['lat']) && isset($_GET['lng']))
            {
                ...
                if (isset($_GET['q']))
                {
                    ...
                    $res = User::leftjoin('user_category', 'users.category_id', '=', 'user_category.id')
                    ->where('users.open', 0)->whereRaw('lower(users.name) like "%' . strtolower($q) . '%"')->where('users.status',0)->where('users.city_id',$city_id);
